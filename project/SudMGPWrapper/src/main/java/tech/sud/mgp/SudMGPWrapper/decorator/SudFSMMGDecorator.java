    ...
    public void onGameStateChange(ISudFSMStateHandle handle, String state, String dataJson) {
        ...
        switch (state) {
            ...
            case SudMGPMGState.MG_COMMON_GAME_INFO_X: // 57. 游戏向app发送获游戏通用数据
                ...
                break;
            case SudMGPMGState.MG_COMMON_AI_MODEL_MESSAGE: // 通知app开启ai大模型
                SudMGPMGState.MGCommonAiModelMessage mgCommonAiModelMessage = SudJsonUtils.fromJson(dataJson, SudMGPMGState.MGCommonAiModelMessage.class);
                if (listener == null) {
                    ISudFSMStateHandleUtils.handleSuccess(handle);
                } else {
                ...
                break;
            case SudMGPMGState.MG_COMMON_AI_MESSAGE: // 通知app ai消息
                SudMGPMGState.MGCommonAiMessage mgCommonAiMessage = SudJsonUtils.fromJson(dataJson, SudMGPMGState.MGCommonAiMessage.class);
                if (listener == null) {
                    ISudFSMStateHandleUtils.handleSuccess(handle);
                } else {
            ...
            case SudMGPMGState.MG_COMMON_AI_LARGE_SCALE_MODEL_MSG: // 64. 通知app ai大模型消息
                ...
                break;
            case SudMGPMGState.MG_COMMON_GAME_PLAYER_MIC_STATE: // 65. 通知app可以开始推送麦克说话状态
                SudMGPMGState.MGCommonGamePlayerMicState mgCommonGamePlayerMicState = SudJsonUtils.fromJson(dataJson, SudMGPMGState.MGCommonGamePlayerMicState.class);
                if (listener == null) {
                    ISudFSMStateHandleUtils.handleSuccess(handle);
                } else {
