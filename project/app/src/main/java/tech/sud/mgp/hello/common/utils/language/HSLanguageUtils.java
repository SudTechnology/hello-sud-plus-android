 ...
 * </pre>
 */
public class HSLanguageUtils {

    private static final String KEY_LOCALE = "KEY_LOCALE";
    private static final String VALUE_FOLLOW_SYSTEM = "VALUE_FOLLOW_SYSTEM";

    private HSLanguageUtils() {
    ...
    public static void applySystemLanguage(final boolean isRelaunchApp, Utils.Consumer<Boolean> consumer) {
        applyLanguageReal(null, isRelaunchApp, consumer);
    }
    ...
    public static void applyLanguageReal(final Locale locale, final boolean isRelaunchApp) {
        applyLanguageReal(locale, isRelaunchApp, null);
    }

    public static void applyLanguageReal(final Locale locale, final boolean isRelaunchApp, Utils.Consumer<Boolean> consumer) {
        if (locale == null) {
    ...
    /**
     * Return whether applied the language by {@link HSLanguageUtils}.
     *
    ...
    /**
     * 如果本地设置了语言，则对当前上下文及application，重新应用一次语言
     *
     * @param context
     */
    public static void fixLanguage(Context context) {
        Locale language = HSLanguageUtils.getAppliedLanguage();
        if (language != null) {
            if (context != null) {
                HSLanguageUtils.attachBaseContext(context);
            }
            HSLanguageUtils.updateAppContextLanguage(language, null);
        }
    }
